...
for(const v of [pkg.version,tauri.version,versionJson.version]) if(v!=='1.13.27') throw new Error('Version drift '+v);
if(!new RegExp(`^version = \"${pkg.version.replace(/\./g,'\\.')}\"$`,'m').test(cargo)) throw new Error('Cargo version drift');
if(!html.includes("var APP_VERSION = '1.13.27';") || !html.includes("version:'1.13.27-RC'")) throw new Error('HTML release version drift');
